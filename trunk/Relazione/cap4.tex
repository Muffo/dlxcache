...
\begin{itemize}
 \item \texttt{Cache\_test\_ReadAndWrite.vhd}: nel quale si verifica il corretto funzionamento delle scritture, nella cache e nella RAM.
 ...
 \item \texttt{Cache\_test\_snoop.vhd}: nel quale si verifica il corretto funzionamento del protocollo MESI in caso di eventuali snoop.
\end{itemize}

Per completezza e per aiutare una successiva lettura delle forme d'onda, date da ISIM durante la simulazione, si riporta fra parentesi dove necessario il valore binario di ci\`o a cui si f\`a riferimento.
...
Si caricano in Cache 3 blocchi da 32 byte, 2 in modalit\`a write-back \texttt{ch\_wtwb='0'} e  uno in modalit\`a write-throght \texttt{ch\_wtwb='1'}. Occupando rispettivamente la via ([x][1]) del primo set di indice ([0][]),del secondo set di indice ([1][x]) e terzo set di indice ([2][]). In particolare nel terzo set si nota come l'attivazione del segnale \texttt{ch\_wtwb}, porti lo stato (\texttt{.status}) del terzo blocco a 1 ovvero a MESI\_S, mentre negli altri due casi \`e uguale a "10" ovvero MESI\_E, differenza evidenziata in verde nella colonna dei valori nella figura \ref{fig:write1}) .
...
Nella prima scrittura, evidenziata in verde nella figura \ref{fig:write2} avviene un miss, in quanto il blocco di TAG="10" non è ancora presente in cache. Verr\`a quindi effettuato:
\begin{itemize}
\item \texttt{}una lettura in RAM per leggere il blocco contenente il dato e portarlo nella seconda via del primo set([0][0]);
\item \texttt{}l'aggiornamento dei valori dei contatori che gestiscono l'invecchiamento, per il meccanismo di rimpiazzamento;
...
Con la prima lettura si verifica che se si effettua una lettura di una word contenuta in un blocco di TAG="0", presente in cache, ovvero abbiamo un hit, il contatore della via([1][1]) dove è contenuto viene resettato, mentre le altre vie con valore più basso di contatore vengono incrementate.

Con la seconda lettura si richiede un dato contenuto in un blocco non presente in cache, ovvero abbiamo  miss, viene quindi selezionato il set in base al valore dell' indice, "01" in questo caso e verr\`a rimpiazzato il blocco con valore del contatore più elevato che in questo caso corrisponde alla via con TAG="10" ( [1][0]) nella quale si era precedentemente resettatato il contatore della via, senza effettuare nessuna scrittura in RAM in quanto il blocco è in stato MESI\_E, ovvero il blocco in cache non ha subito nessuna modifica rispetto a quanto presente in RAM.

Con la terza lettura invece si richiede un dato contenuto in un blocco non presente in cache con indice="10", che corrisponde al terzo set ([2][x]), nel quale una via era stata precedentemente invalidata, via([2][0]) che verr\`a quindi rimpiazzata, senza scrittura in RAM, con il blocco con TAG="110".
...
Nel terzo caso, evidenziato in verde in Fig.\ref{fig:snoop1}, si effetua uno snoop su di un blocco che \`e presente in cache in stato MESI\_M("11"), la cache  quindi:  
\begin{itemize}
\item \texttt{}porter\`a \texttt{ch\_hit="1" e ch\_hitm="1"}
\item \texttt{}effettuer\`a la scrittura in RAM del blocco contenuto in cache
\item \texttt{}porter\`a lo stato della via in MESI\_S("1")
\end{itemize}
