...
\begin{itemize}
  \item l3) \texttt{0(r1)=0+0= "000 00 00000"} ovvero prima([1]) via occupata nel primo set
  \item l5) \texttt{16\#100(r1)="010 00 00000"} ovvero seconda([0]) via occupata nel primo set
  \item l7) \texttt{16\#80(r1)= "001 00 00000"} ovvero prima([1]) via rimpiazzata nel primo set, quindi si ha una scrittura in RAM del blocco modificato e caricamento del nuovo blocco.
\end{itemize}
...
In seguito si effettua il caricamento in r2 del primo valore scritto in memoria (ovvero "1"), ma il blocco che contiene la word di indirizzo 0 non \`e pi\`u presente in cache, verra quindi effettuato un ulteriore rimpiazzamento però sulla seconda via([0]).\\
