...
\begin{itemize}
 ...
 \item \texttt{Cache_test_ReadAndReplacement.vhd}: nel quale si verifica il corretto funzionamento della politica di rimpiazzamento mediante contatori.
...
Nella prima scrittura (evidenziata in verde nell'immagine) avviene un miss, in quanto il blocco di TAG="10" non è ancora presente in cache, verrà quindi effettuato:
\begin{itemize}
...
\item \texttt{} verrà effettuato una scrittura in cache aggiornando il valore della word di offset "00110" (ovvero il byte dalla posizione [6] alla [9]), e lo stato\texttt{.status}\\ della via viene posto a MESI_M ("11").
\end{itemize}
...
Nella terza scrittura  (evidenziata in azzurro nell'immagine) a differenza dei due casi precedenti, la via \`e stata portata in cache in modalita write-throught di conseguenza, la scrittura oltre ad avvenire in cache, avviene anche in RAM, ed essendo il nostro caso a monoprocessore la via anzichè porsi in stato MESI_M, come negli altri casi si pone in MESI_E in quanto il contenuto del blocco in cache è uguale a quello a ciò che c'\`e al livello superiore (nel nostro caso la RAM).
...
In questa ultima fase vengono eseguite delle letture; nel primo caso due letture per verifare i dati scritti nelle prime due scritture.
...
Si analizzer\`a infine anche il caso in cui la cahce subisca un invalidazione di una linea.

\subsubsection{Fase 1: Letture d' inizializzazione}
Nella prima fase si effettuano 8 letture per riempire tutta la cache, quindi  tutti i contatori nelle vie, dei quattro set, che sono stati caricati per ultimi con il contatore (\texttt{lru_counter }\\) a "0",mentre le altre vie hanno il contatore a "1".

\subsubsection{Fase 2: Invalidazione}
Nella seconda fase si esegue un invalidazione sul blocco di TAG="010" e con index="10", ovvero il terzo set; questo comporta il portare lo stato\texttt{.status}\\ da MESI_E(10) a MESI_I(0) e contatore a "1" nella via che lo contiene mentre l'altra via del set si portata a "0".
...
Con la terza lettura invece si richiede un dato contenuto in un blocco non presente in cache con indice="10", che corrisponde al terzo set nel quale una via era stata precedentemente invalidata, via che verra quindi rimpiazzata, senza scrittura in RAM, con il blocco con TAG="110".
...
Quindi si procede con l'inizializzazione della cache per permettere di verificare il corretto funzionamento dello snoop nei vari casi. Nello specifico si portano prima in cache due blocchi dalla RAM e sul secondo blocco si esegue una scrittura per portarlo in stato MESI_M.
In secondo luogo si attiva il segnale di snoop ovvero:\texttt{ch\_eads}\\ e siimpostata l'indirizzo su cui fare lo snoop:\texttt{ch\_snoop\_addr}\\. La cache risponderà in modo opportuno con i segnali di \texttt{ch\_hit,ch\_hitm}\\ e modificando se il dato \`e presente in cache lo stato delle vie.

Nel primo caso si effettua uno snoop su di un blocco che non \`e contenuto in cache, quindi la cache risponde con  \texttt{ch\_hit="0",ch\_hitm="0"}\\.

Nel secondo caso(evidenziato in giallo in Fig.\ref{fig:snoop1}) si effettua uno snoop su di un blocco presente in cache in stato MESI_E, quindi la cache risponde con \texttt{ch\_hit="1",ch\_hitm="0"}\\ e porta lo stato della via interessata a MESI_S("1").

Nel terzo caso(evidenziato in verde in Fig.\ref{fig:snoop1}) si effetuer\`a uno snoop su di un blocco che \`e presente in cache in stato MESI_M("11"), la cache  quindi:  
\begin{itemize}
\item \texttt{}\texttt{porta ch\_hit="1" e ch\_hitm="1"}\\
\item \texttt{}\texttt{effettua la scrittura in RAM del blocco contenuto in cache.}\\
\item \texttt{}\texttt{portan lo stato della via in MESI_S("1")}\\
\end{itemize}
...
\end{lstlisting} 
Questo codice conta fino a tre (da qui il nome del file termina con 123), e ad ogni iterazione ricomincia da uno, inoltre ogni risultato intermedio viene salvato in memoria in una locazione diversa in modo da obbligare la cache ad ogni iterazione di rimpiazzare un blocco sempre lavorando nel primo set per un certo numero d'iterazioni , infatti ad esempio nella prima iterazione quando r1="0",si nota che i due bit che identificano l'indice sono sempre uguali a "00":
...
Si continua a operare nel primo set finc\`e il valore di r1 non supera 31, ovvero alla 32esima iterazione si passer\`a a lavorare sul secondo set.

In seguito si effettua il caricamento in r2 del primo valore scritto in memoria (ovvero "1"), ma il blocco che contiene la word di indirizzo 0 non \`e pi\`u presente in cache, verra quindi effettuato un ulteriore rimpiazzamento però sulla seconda via([0]). infine si incrementa in valore di r1 di quattro unita in modo da scrivere non pi\`u nella word di offset "00000" ma di "00010", ovvero la word successiva di ogni blocco.
...
\begin{itemize}
   \item \texttt{l'istruzione critica} in giallo;
	\item \texttt{i valori forniti dalla Forwarding Unit e il risultato corretto} in verde;
	\item \texttt{i valori forniti non aggiornati forniti dallo stadio di decode} in rosso.
\end{itemize}
