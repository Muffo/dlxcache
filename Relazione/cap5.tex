...
In questa fase abbiamo riscontrato che durante ogni operazione in cui si richiede di portare in uscita un dato presente sulla Block Ram ad un certo indirizzo (quindi durante le lettura, ma anche durante le scrittura per effetto delle WRITE\_MODE da gestire), il dato viene reso disponibile in uscita al clock successivo e dopo un ritardo di 100 ps. L'effetto \`e pertanto che al primo clock si richiede una dato e lo si ottiene solo al secondo quando è in corso la richiesta del secondo dato. La Fig.\ref{fig:lettura_ritardata} mostra in dettaglio ci\`o che accade. \texttt{br\_out} \`e il segnale di debug utilizzato per mostare il byte in uscita alla Block Ram dopo ogni richiesta.\\
\begin{figure}[!h]
\centering
