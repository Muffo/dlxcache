...
\label{fig:tipi_br}
\end{figure}
Analogamente avviene per l'utilizzo di Block Ram Dual-Port dove l'organizzazione dei dati acceduti dalle due porte è rappresentato dai suffissi mostrati in Tabella \ref{fig:suffixDP}.
\begin{figure}[!h]
\centering
%\includegraphics[width=\textwidth]{img/blockRam/tabTipiRam.jpg}
...
\item La dimensione in byte della linea di memoria è configurabile impostando opportunamente il numero di bit di OFFSET dell'indirizzo.
...
\begin{enumerate}
\item \texttt{write\_line}: la scrittura di una linea in Block Ram deve provvedere al trasferimento byte per byte della linea in ingresso su \texttt{bdata\_in} sulla Block Ram tramite una serie di \texttt{2**OFFSET\_BIT} scritture consecutive che avvengono sul fronte positivo del clock \texttt{clk} in ingresso alla Block Ram.
...
In questa fase abbiamo riscontrato che durante ogni operazione in cui si richiede di portare in uscita un dato presente sulla Block Ram ad un certo indirizzo (quindi durante le lettura, ma anche durante le scrittura per effetto delle WRITE\_MODE da gestire), il dato viene reso disponibile in uscita al clock successivo e dopo un ritardo di 100 ps. L'effetto \`e pertanto che al primo clock si richiede una dato e lo si ottiene solo al secondo quando è in corso la richiesta del secondo dato. La Fig.\ref{fig:lettura_ritardata} mostra in dettaglio ci\`o che accade. \texttt{br\_out} \`e il segnale di debug utilizzato per mostare il dato in uscita dalla Block Ram.\\
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{img/blockRam/letturaRitardata.png}
\caption{Dato richiesto ad un clock è reso disponibile in uscita al clock successivo + ritardo di 100 ps.}
\label{fig:lettura_ritardata}
\end{figure}
Consultando la User Guide di Xilinx \cite{xil2} non abbiamo raccolto informazioni in merito alle temporizzazioni in lettura per avere un dato in uscita (dopo averne comandato una lettura), pertanto abbiamo cercato una spiegazione su forum del settore e abbiamo riscontrato che i due clock per avere il dato in uscita sono necessari inquanto per alcune FPGA la scrittura sul registro di output avviene al ciclo successivo dopo che il dato \`e stato richiesto e il ritardo di 100 ps \`e legato al fatto che la Block Ram \`e un componente reale e tale ritardo \`e utilizzato per simularne l'accesso.\\
