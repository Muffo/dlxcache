...
Con la seconda lettura si richiede un dato contenuto in blocco non presente in cache, ovvero abbiamo  miss, viene quindi selezionato il set in base al valore dell' indice, "01" in questo caso e verr\`a rimpiazzato il blocco con valore del contatore più elevato che in questo caso corrisponde alla via con TAG="10" ( [1][0]) nella quale si era precedentemente resettatato il contatore della via, senza effettuare nessuna scrittura in RAM in quanto il blocco è in stato MESI\_E, ovvero il blocco in cache non ha subito nessuna modifica rispetto a quanto presente in RAM.
